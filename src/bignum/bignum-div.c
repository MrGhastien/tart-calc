...
static void remainder(bignum* x, bignum* y, u32 k) {
    bnReserve(x, 1);
    u32 carry = 0;
...
static u16 trial(bignum* r, bignum* d, u32 k, u32 m) {
    u64 km = k + m;
    u64 r3 = ((u64)getWord(r, km) << (RADIX_POWER + RADIX_POWER))
        + (getWord(r, km - 1) << RADIX_POWER)
        + getWord(r, km - 2);
...
void bnDiv(bignum* dividend, const bignum* divisor, bignum* outRemainder) {
    i32 offsetA;
    i32 offsetB;
    ...
    if (divisor->size == 1) {
        remainder(outRemainder, dividend, dCpy.words[0]);
